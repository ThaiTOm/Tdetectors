...
def cc_pintel_download_local():
    cfg_path = cfg.roboflow.cc_pintel
    dataset = dataset_download_local(cfg_path)
    return dataset

def download_dataset_vehicle():
    # This function is intended to download the dataset from Roboflow.
    # Implement the download logic here.
    cfg_path = cfg.roboflow.vehicle
    for each_dataset in cfg_path:
        dt = cfg.roboflow.vehicle[each_dataset]
        print(f"Downloading dataset from Roboflow.... The information is:"
              f"\nWorkspace: {dt.workspace}, ")

        print(f"Project: {dt.project}, Version: {dt.version}, YOLO Model: {dt.yolo_model}")

        dataset = dataset_download_local(dt)
        ...
        for split in os.listdir(dataset_path):
            ...
            if os.path.isdir(split_path):
                print(f"Found split: {split} with {len(os.listdir(split_path))} images.")
                label_change(os.path.join(split_path, "labels"))
                print(f"Processed labels in split: {split}")

def vehicle_joffa_download_local():
    cfg_path = cfg.roboflow.vehicle_joffa
    dataset = dataset_download_local(cfg_path)
    return dataset

def vehicle_joffa_process_raw_data():
...
def push_dataset_to_roboflow():
    ...
    PROJECT_URL = "detectors-vehicle"  # e.g., "vehicles-detection"

    DATA_FOLDER_PATH = "/home/geso/Tdetectors/data/VehicleDataset/Car-Counting-1"
