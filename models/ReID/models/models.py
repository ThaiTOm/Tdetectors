...
def load_weights_custom(model, weights_path, new_class_num):
    """
    Loads pre-trained weights into a model, ignoring the final classifier layer
    if the number of classes is different.

    Args:
        model (nn.Module): The new model instance (with the new class_num).
        weights_path (str): Path to the .pth or .pt file containing the pre-trained weights.
        new_class_num (int): The number of classes for the new model.
    """
    ...
    try:
        ...
        # If the weights are saved within a key (e.g., 'model_state_dict'), extract them
    ...
    # 1. Filter out weights from the pre-trained dict that are not in the new model
    #    or have a shape mismatch (this will be our classifier layers).
    #    This step is implicitly handled by `load_state_dict` with `strict=False`,
    #    but we can do it explicitly for clarity and to see what's being skipped.
    ...
    if mismatched_keys:
        print("Skipping weights for the following layers due to mismatch (this is expected for the classifier):")
        for key in mismatched_keys:
            # We are primarily interested in the classifier weights
            if 'classifier' in key:
                print(f"  - {key}")
    else:
        print("All layers matched perfectly.")

    # 2. Overwrite the new model's state dict with the filtered pre-trained weights
    model_dict.update(pretrained_dict_filtered)

    # 3. Load the updated state dictionary into the new model
